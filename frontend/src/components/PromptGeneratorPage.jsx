...
export default function PromptGeneratorPage() {
  ...
  const EXAMPLES = ['make website fast','doctor appointment app','marketing email for shoes','मुझे chatbot बनाना है','résumé for data scientist','legal contract review'];
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'1.5rem', gap:'1.25rem', overflow:'auto', maxWidth:'900px', margin:'0 auto', width:'100%' }}>
      ...
      <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
        {EXAMPLES.map(ex => (
          <button key={ex} onClick={() => { setInput(ex); reset(); }} style={{ fontSize:'11px', padding:'5px 12px', background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:'100px', color:'var(--tx-secondary)', cursor:'pointer', fontFamily:'var(--font-ui)', transition:'all 0.15s' }}
      ...
      <div style={{ background:'var(--bg-panel)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', overflow:'hidden' }}>
        ...
        <div style={{ padding:'10px 14px', background:'var(--bg-surface)', borderTop:'1px solid var(--border-dim)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <div style={{ display:'flex', gap:'8px' }}>
            {input && (
              <button onClick={() => { setInput(''); reset(); }} style={{ padding:'7px 14px', background:'transparent', border:'1px solid var(--border)', borderRadius:'var(--r-md)', color:'var(--tx-muted)', fontSize:'12px', cursor:'pointer', fontFamily:'var(--font-ui)' }}>Clear</button>
            )}
            <button onClick={handleGenerate} disabled={isLoading || !input.trim()}
              style={{ display:'flex', alignItems:'center', gap:'7px', padding:'7px 18px', background: isLoading || !input.trim() ? 'var(--bg-hover)' : 'linear-gradient(135deg, #bc8cff, #58a6ff)', border:'none', borderRadius:'var(--r-md)', color: isLoading || !input.trim() ? 'var(--tx-secondary)' : '#0d1117', fontSize:'13px', fontWeight:700, cursor: isLoading || !input.trim() ? 'not-allowed' : 'pointer', fontFamily:'var(--font-ui)', transition:'all 0.2s' }}>
      ...
      {result && !isLoading && (
        <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
          ...
          <PromptCard title="✦ Generated Prompt"  subtitle="Main prompt — ready to use"               content={result.generatedPrompt} accentColor="#bc8cff" onCopy={() => copyText(result.generatedPrompt, 'main')}     copied={copiedKey === 'main'} />
          <PromptCard title="⟳ Prompt Variants"   subtitle="Alternative versions — different angles"   content={result.promptVariants}  accentColor="#58a6ff" onCopy={() => copyText(result.promptVariants,  'variants')} copied={copiedKey === 'variants'} />
          <PromptCard title="💡 Tips to Improve"  subtitle="Make your prompt even better"              content={result.tips}            accentColor="#e3b341" onCopy={() => copyText(result.tips,            'tips')}     copied={copiedKey === 'tips'} />
        </div>
      )}
    </div>
  );
